      ...
      children: [
        ...
        ...(resumeData.personalInfo.summary ? [
          new Paragraph({
            ...
            spacing: { after: 100 },
            border: templateName === 'modern' || templateName === 'bold' ? {
              bottom: { color: template.titleColor || '2563EB', space: 1, style: BorderStyle.SINGLE, size: 6 },
            } : undefined,
          }),
          new Paragraph({
...
const renderBoldTemplate = (doc: jsPDF, data: ResumeData) => {
    ...
    if (codingProfilesEntries.length > 0) {
        ...
        codingProfilesEntries.forEach(([platform, url]) => {
            ...
            doc.setFont("helvetica", "bold");
            doc.setTextColor(220, 38, 38);
            doc.text(`${platform.charAt(0).toUpperCase() + platform.slice(1)}:`, margin, yPos);
